...
export function App() {
    ...
    try {
      const created = await api.addMember({
        ...
        ...(newMember.kind === "chatbot" ? {
          ...
          apiUrl: newMember.apiUrl.trim() || undefined,
  ...
  return <main className="app-shell" ref={frame.rootRef} style={{ ...frame.rootStyle, display: "flex", flexDirection: "column" }} data-left={frame.leftMode} data-right={frame.rightOpen ? "open" : "closed"}>
    ...
    <Shell
      ...
      header={current ? <>
          <button className="icon-button mobile-nav" onClick={frame.toggleLeft} aria-label="群列表">☰</button>
      ...
      composer={current && mainView !== "dsh" && mainView !== "agent-config" && !(!isChatGroup && mainView === "versions") ? <div className="wp-composer-anchor">
          ...
          <Composer
            ...
            textarea={<textarea ref={composerRef} rows={2} value={composer} onChange={(event) => { const value = event.target.value; setComposer(value); setSlashOpen(value.startsWith("/") && !value.includes(" ")); }} onKeyDown={composerKeyDown} onPaste={handlePaste} placeholder={`给群里的 Agent 派活…  @ 提及 · ${sendKeyHint(sendKeyMode)} · 草稿自动保存`} />}
            hint={<>
          ...
          {addMemberKind === "agent" && <>
            ...
            {isConnecterRemote ? <>
              ...
              <input
                ...
                placeholder="Group Ref，例如 wp:ecs-canary:seed-group-ohmyworkpanel"
                required
              />
              <input
