  ...
  return <main className="app-shell">
    ...
    {current && showMembers && <aside className="member-panel">
      ...
      {showAddMember ? <form className="add-member-form" onSubmit={addMember}>
        ...
        {newMember.kind === "agent" && <><select value={newMember.adapter} onChange={(event) => setNewMember((value) => ({ ...value, adapter: event.target.value }))}><option value="mock">模拟 Agent（推荐体验）</option><option value="codex">Codex CLI</option><option value="claude-code">Claude Code</option><option value="opencode">OpenCode（预留）</option><option value="cursor">Cursor（预留）</option></select><input value={newMember.executablePath} onChange={(event) => setNewMember((value) => ({ ...value, executablePath: event.target.value }))} placeholder="可执行文件路径（可选）" /></>}
