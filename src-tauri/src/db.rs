...
pub fn get_connecter_cancel_target(
    ...
    let adapter: Option<String> = connection
        ...
        .map_err(|e| e.to_string())?;
    if adapter.as_deref()
        != Some(crate::agents::adapters::connecter_remote::ADAPTER_ID)
    {
        return Ok(None);
    }
    Ok(Some((
...
mod tests {
    ...
    fn connecter_provider_profile_is_encrypted_cascades_and_anchors_dispatch() {
        ...
        set_task_run_provider_dispatch_id(&conn, "r", "dispatch-1").unwrap();
        assert_eq!(
            get_task_run_provider_dispatch_id(&conn, "r")
                .unwrap()
                .as_deref(),
            Some("dispatch-1")
        );
        ...
        conn.execute("DELETE FROM task_runs WHERE id='r'", [])
            .unwrap();
        conn.execute("DELETE FROM members WHERE id='a'", [])
            .unwrap();
