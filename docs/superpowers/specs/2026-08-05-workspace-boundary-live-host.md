...
| 树 | 路径 | 职责 |
|---|---|---|
| Host | `/AI/ohMyWorkPanel` | 代理、`LivePanel`、短回复注入、A2A、群/成员 |
| Extend | `/AI/WorkPanelLive` | `:8790`、`live.html`、STT/TTS/DashScope |

Host 经 `OHMYWORKPANEL_PANELLIVE_ROOT`（默认 `/AI/WorkPanelLive`）读清单 — **路径耦合，不是混仓**。
...
| ohMyWorkPanel | 平台种子群；`/AI/ohMyWorkPanel` |
...
2. **改码规矩（写死）**  
   ...
   - Host 代理 / `LivePanel` / 短回复注入 / A2A → **只动** `/AI/ohMyWorkPanel`
