...
> 本文是「把 DSH 的自举能力接入 WorkPanel」的**设计主文档**。
...
WorkPanel 的自我更新是**群聊驱动的产品迭代**：群聊里讨论 → 版本/Wave 立项 → 灰度 :8081 → 审批 → promote :8080。它强在**人机协作与治理**（组织、审批、交接、审计），弱在**运行时自举原语**：会话不是机器可回放的分叉点、扩展是静态 manifest、回滚靠流程而非系统原语。
...
两者天然互补：**WorkPanel 提供「为什么做、谁批准、怎么保证不失控」，DSH 提供「能做出什么、能否留痕、能否回滚」。**

## 设计原则（先定边界）

1. **借鉴而非合并**：不把 DSH 内核寄生进 WorkPanel 运行时。DSH 是**外部可换执行运行时**（进程隔离），经 CLI / ACP / 同源代理 HTTP 桥接。
2. **双平面分工**：
   - **治理 / 协作平面（WorkPanel）**：群聊议事、`@` 调度、角色/管理员、版本/Wave、灰度→审批→promote。
...
6. **两级自举执行者（每组一个、不可修改）**：
   ...
   - **WorkPanel 组**（种子/系统群，`is_system=1`）：预制 **`linlis-super-harness`**（超级 harness）——**只有它拥有“面板自己改自己”的完整自举能力**（提案→干跑→写回→灰度→上线）。两者均 `system_locked=1`、不可编辑/移除。

## 三层架构

```text
[意图/决策层]   群聊（议事 + 审批 + 叙事）           ← WorkPanel 负责
      │ 关联决策卡 / 提案
[结构化状态层]  版本/Wave · diff · 审批单 · 会话重放   ← WorkPanel 负责（可结构化视图）
      │ 驱动 / 观察
...
| # | DSH 原语 | 现状（WorkPanel） | 借鉴后（WorkPanel） | 优先级 |
|---|---|---|---|---|
...
```text
WorkPanel 组：管理员/成员提出「给面板加能力/改规则」并 @linlis-super-harness
  ↓ 立项（版本/Wave）
...
- **WorkPanel 组**：`linlis-super-harness`（超级 harness，WorkPanel 自我更新唯一完整引擎）。

### 预制（随创建即自带，两级模板）
- **每个群**（普通项目群 / WorkPanel 组）在 seed / 创建时自动带一个自举 Agent；Web 与桌面同一套 seed 逻辑。
...
- **WorkPanel 组模板**：`id=linlis-super-harness`、`displayName=linlis-super-harness`、`adapter=dsh`（super profile）；能力 = 完整自举（提案→干跑→写回 manifest/扩展→提交灰度）。
...
### 唯一自举通道（分两级权限）
- **完整自举写回权只属于 WorkPanel 组的 `linlis-super-harness`**：写回 manifest/扩展、提交灰度申请、驱动“面板自己改自己”。
...
### 鲁棒性
- 两级 bootstrap（极简 bootstrap-dsh / WorkPanel 组 linlis-super-harness）都依赖 dsh 运行时（headless / ACP）；需健康检查与就绪态展示。
...
| **P1** | ACP 长驻适配器 + session 事件回灌 WorkPanel 消息/run | 可回放会话 ① | 待立项 |
| **P2** | 预制**两级模板**：普通群极简 `bootstrap-dsh-<group>` + WorkPanel 组 `linlis-super-harness`（均 `system_locked` 不可改 + 分级权限）+ 扩展宿主能力化（可逆注册） | 可逆 effect ② + 能力 seam ③ | 待立项 |
| **P3** | 自举全闭环：WorkPanel 组群聊 `@linlis-super-harness` 发起「面板自改」→ 版本/Wave → 灰度（热载 dsh 能力）→ 审批 promote → 可回滚 | 全闭环 | 待立项 |
...
6. **两级 bootstrap 不可改性**：`system_locked=1` 平台锁定；前端只读、后端 mutation 拒绝；完整自举写回权只挂 WorkPanel 组 `linlis-super-harness`，普通群极简 `bootstrap-dsh` 无写回权，均不可被普通 Agent 复制。
...
- **DSH 破坏性变更**：锁版本；把「DSH 依赖」当作可替换件，不进入 WorkPanel 长期编译期契约。
- **会话回放的数据量**：run 级全量留痕需定分块与会话窗口（参考滚动摘要），避免存崩溃。
- 开放问题：
- **单点执行风险**：完整自举只走 WorkPanel 组的 linlis-super-harness，若 dsh 缺失/退化则自举暂停。缓解：健康检查 + fail-closed 只对自举、fail-open 对日常；super harness 可独立重装但身份固定。
  - P1 的 session 回灌是「落库镜像」还是「仅保留 replay 句柄」？
  - P2 的「能力热载」边界：只允许扩展宿主能力（工具/页签），还是允许改 WorkPanel 本体代码？
  - 普通群极简 bootstrap-dsh 是否允许“跨群提案”（把本群诉求上报给 WorkPanel 组）？
