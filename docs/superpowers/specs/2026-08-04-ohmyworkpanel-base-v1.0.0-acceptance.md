# ohMyWorkPanel BaseV1.0.0 — 现状梳理与验收标准

日期：2026-08-04  
路线图项：`ohMyWorkPanel BaseV1.0.0发布`（`41142775-7693-49df-a059-85bf9d5186da`）
本文件对应 checklist：`梳理现状与验收标准（ohMyWorkPanel BaseV1.0.0发布）`

## 1. 产品定位（Base 范围）

ohMyWorkPanel BaseV1.0.0 指「可日常使用的多 Agent 协作工作台」基线，而非全部未来能力（如 A2A 协议另列为后续 Feature）。
...
- 数据：`/AI/ohMyWorkPanel/data`（prod）与 `data-canary`（灰度）分离 — 不得互相覆盖。
- 服务：`ohmyworkpanel`、`ohmyworkpanel-canary`、`linlis-codex-proxy` 均为 active（代理提供 Codex→`:18888` Responses shim）。
...
下列全部勾选后，可将路线图项 `ohMyWorkPanel BaseV1.0.0发布` 标为 **completed**。
...
- [x] root/root 登录；群 **ohMyWorkPanel** 可见
...
- [x] `http://127.0.0.1:8080/` → 200；root 登录；**ohMyWorkPanel** 群与历史仍在
...
1. 本文档已写入仓库路径 `docs/superpowers/specs/2026-08-04-ohmyworkpanel-base-v1.0.0-acceptance.md`
