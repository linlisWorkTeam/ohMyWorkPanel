...
交付可运行的 **PanelLive Extend 服务骨架（方案 A）**：本地 Mock STT/TTS + 类微信语音页，并沉淀对 WorkPanel 平台的 Extension / A2A 要求。不在本 MVP 调用真实云 STT/TTS。
...
| 平台要求文档 | `LinlisWorkPanel/docs` + 群路线图 + A2A 群消息 |
...
3. 对 WorkPanel 的 API/协议需求清单  
4. WorkPanel 群路线图写入平台待办，并用 `@` 通知 Agent  

## Non-goals

- 真实 dashscope / NLS  
- WorkPanel Extension Host 完整实现（由平台侧后续做）  
- Connecter 媒体中转  

## Success criteria

- `npm start` 后可打开 Live 页，按住说话 → 看到 Mock 文本 → 听到/下载 Mock 音频  
- docs 与路线图中平台要求可被 WorkPanel Agent 认领  
